...
municipality_names = """1440 	Ale kommun
...
code_to_name = {}
for line in municipality_names.split("\n"):
    components = line.split("\t")
    code_to_name[components[0].strip()] = components[1].strip()
...
def filterTags(attrs):
    ...
    # Look up the real name from the array
    if len(res['ref:scb']) == 4:
        res['short_name'] = res['name']
        res['name'] = code_to_name[res['ref:scb']]
    
    # Append Län at thet end to make it into a county.
    if len(res['ref']) == 2:
        res['name'] += ' Län'
